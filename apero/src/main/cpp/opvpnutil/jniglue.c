...
void Java_com_apero_openvpn_core_NativeUtils_jniclose(JNIEnv *env,jclass jo, jint fd)
{
	int ret = close(fd);
}
...
jstring Java_com_apero_openvpn_core_NativeUtils_getJNIAPI(JNIEnv *env, jclass jo)
{

    return (*env)->NewStringUTF(env, TARGET_ARCH_ABI);
}

jstring Java_com_apero_openvpn_core_NativeUtils_getOpenVPN2GitVersion(JNIEnv *env, jclass jo)
{

  return (*env)->NewStringUTF(env, OPENVPN2_GIT_REVISION);
}

jstring Java_com_apero_openvpn_core_NativeUtils_getOpenVPN3GitVersion(JNIEnv *env, jclass jo)
{
